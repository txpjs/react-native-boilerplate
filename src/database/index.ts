...
export default class BaseTable<T> {
  ...
  create({ params, callback }: Create<T, BaseTableType>) {
    ...
    this.db.transaction(
      (tx) => {
        tx.executeSql(`insert into ${this.tableName} (obsolete, ${keys}) values (0, ${keysMap})`, values, () => {
          callback();
        });
      },
      () => {
  ...
  update({ params, callback }: Update<T, BaseTableType>) {
    ...
    const str = JSON.stringify(rest).replace('{', '').replace('}', '').replace(new RegExp(':', 'gm'), ' = ');
    this.db.transaction(
      (tx) => {
  ...
  read({ params, callback }: Read<T>) {
    ...
    this.db.transaction(
      (tx) => {
        tx.executeSql(`select * from ${this.tableName} where obsolete = 0 ${where};`, [], (_, { rows: { _array } }) => {
          if (callback) {
            callback(_array);
          }
        });
